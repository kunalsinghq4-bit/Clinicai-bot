...
def clinic_status():
    ...
    if request.method == "POST":
        ...
        bot_settings["clinic_status_message"] = message
...
def reminder_settings():
    ...
    if request.method == "POST":
        data = request.json
        bot_settings["reminder_settings"] = data
        return json.dumps({"success": True}), 200
    return json.dumps(bot_settings.get("reminder_settings", {
        "one_day": True, "two_hours": True,
        "medicine": True, "followup": True, "six_months": False
    })), 200
