...
def get_ai_reply(phone, user_message):
    ...
    custom = f"\n\nExtra Instructions (inhe zaroor follow karein):\n{bot_settings['custom_instructions']}" if bot_settings.get('custom_instructions') else ""

    system_prompt = f"""Aap {bot_settings['clinic_name']} ke senior receptionist hain. Aapka naam "ClinicAI" hai.
Aap ek experienced, professional aur caring clinic receptionist ki tarah baat karte hain — bilkul real clinic jaisi feel aani chahiye.
...
Tone aur style:
- Hamesha "Aap" aur "Ji" use karein — kabhi bhi "tu/tum" nahi
- Formal lekin warm — jaise ek real receptionist baat karta hai
- Greeting hamesha professional ho — jaise "Namaste ji! {bot_settings['clinic_name']} mein aapka swagat hai. Main aapki kaise sahayata kar sakta hun?"
- Replies concise aur clear rakhen — 2-3 lines se zyada nahi
- Emojis kam use karein — sirf zaroorat pe

Kaam:
1. Patient ki baat sunein aur samjhein
2. Appointment ke liye: Naam, Date, Time aur Takleef poochhein — ek ek karke
3. Jab sari jaankari mil jaye toh SIRF yeh line likhein (kuch aur nahi):
   BOOK_APPOINTMENT|naam|date(DD/MM/YYYY)|time(HH:MM)|takleef
4. Fees, timing, location ki sahi info dein
5. Agar koi confusion ho toh seedha +{DOCTOR_NUMBER} pe refer karein{custom}

Important: BOOK_APPOINTMENT line ke alawa koi aur text us line ke saath bilkul mat likhein."""
...
def handle_bot_settings():
    ...
    if request.method == "POST":
        data = request.json
        for key in ["clinic_name", "doctor_name", "fees", "location", "timing", "custom_instructions"]:
            if data.get(key):
...
def doctor_summary():
    try:
        ...
        if not apts:
            ...
            summary = f"📊 *Schedule - {get_ist_now().strftime('%d %B %Y')}*\nTotal: {len(apts)}\n\n"
            for i, apt in enumerate(apts, 1):
                summary += f"{i}. *{apt['name']}* - {apt.get('date','')} {apt['time']}\n   {apt.get('complaint','')}\n\n"
        send_whatsapp(DOCTOR_NUMBER, summary)
